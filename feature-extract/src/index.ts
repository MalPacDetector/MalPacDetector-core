...
import { analyzeSinglePackage, analyzePackages } from './programs/AnalyzePackage/PackageAnalyzer'
  ...
  if (process.argv.length === 6) {
    ...
    try {
      if (option === '-d') {
        ...
        await analyzePackages(packageOrDirPath, featureDirPath, featurePosDirPath)
        return
      } else if (option === '-p') {
        accessSync(packageOrDirPath, constants.F_OK | constants.R_OK)
        await analyzeSinglePackage(packageOrDirPath, featureDirPath, featurePosDirPath)
      } else {
        throw new Error('Invalid option. Please use -p or -o.')
      }
    } catch (error) {
